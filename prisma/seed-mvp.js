...
// Manually load .env because we don't want to rely on external libs or CLI quirks
...
async function main() {
    ...
    const toolSlack = await prisma.tool.upsert({
        where: { slug: 'slack' },
        update: {},
        create: {
            name: 'Slack',
            slug: 'slack',
            description: 'Communication for teams.',
    ...
    const strategyGlass = await prisma.strategy.upsert({
        ...
        update: {
            // Force update relation just in case
            tools: {
